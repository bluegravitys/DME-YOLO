...
from ultralytics.nn.Extramodule.ODConv import ODConv, C2f_ODConv
from ultralytics.nn.Extramodule.SPDConv import SPDConv
from ultralytics.nn.Extramodule.SELA import SELA, C2f_SELA
import contextlib
from copy import deepcopy
from pathlib import Path
from ultralytics.nn.Extramodule.MobileNetV4 import MobileNetV4ConvMedium, MobileNetV4HybridMedium, \
    MobileNetV4HybridLarge, MobileNetV4ConvSmall, MobileNetV4ConvLarge
import torch
from ultralytics.nn.Extramodule.EfficientNetV2 import *
from ultralytics.nn.Extramodule.TripletAttention import *
from ultralytics.nn.Extramodule.BiFomer import *
import torch.nn as nn
from ultralytics.nn.Extramodule.AKConv import AKConv
from ultralytics.nn.Extramodule.CBAM import CBAM
from ultralytics.nn.modules import (BasicStage, PatchEmbed_FasterNet, PatchMerging_FasterNet)
import pdb
from ultralytics.nn.Extramodule.WTconv import *
from ultralytics.nn.Extramodule.EAMAattention import *
...
def torch_safe_load(weight):
    ...
    try:
        with temporary_modules(
            ...
            # PyTorch>=2.6 默认 `weights_only=True` 会导致部分 Ultralytics 旧权重加载失败。
            # 显式关闭以保证兼容。
        ...
        # 同上：显式关闭 weights_only 以保证权重可正确反序列化
        ckpt = torch.load(file, map_location="cpu", weights_only=False)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB, C2f_SELA,C2f_WTConv):
                args.insert(2, n)  # number of repeats
                n = 1
        elif m in {TripletAttention,EAMA}:
            c2 = ch[f]
            args = [c2, *args]
